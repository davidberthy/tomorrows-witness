...
async function callClaude(system, messages, useSearch = false, model = "claude-sonnet-4-6") {
  const body = {
    model,
    max_tokens: 1000,
    system,
...
async function handleFollowUp(messages, statusCb) {
  ...
  const result = await callClaude(conversationSystem, claudeMessages, false, "claude-opus-4-6");
    ...
    <div style={{ padding: "0 16px", marginBottom: 12 }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
          ...
          >
            <span style={{ display: "inline-block", marginRight: 8, verticalAlign: "middle" }}><SeedGlyph size={18} animate={false} /></span>The Traveler · {DATES.futureYear}
          </span>
        </div>
        <div
          key={status}
...
export default function TomorrowsWitness() {
  ...
  const metaculus = markets.filter((m) => m.source === "Kalshi");
    ...
    >
      ...
      {view === "signals" && (
        ...
        >
          ...
          >
            AI-curated from{" "}<a href="https://polymarket.com" target="_blank" rel="noopener" style={{color:"var(--amber)",textDecoration:"none",borderBottom:"1px solid var(--amber-dim)"}}>Polymarket</a>{" "}and{" "}<a href="https://kalshi.com" target="_blank" rel="noopener" style={{color:"var(--amber)",textDecoration:"none",borderBottom:"1px solid var(--amber-dim)"}}>Kalshi</a> — the most strategically interesting questions people are betting real money on right now.
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
          ...
          <div style={{ position: "relative", maxHeight: "85vh", overflowY: "auto", background: "linear-gradient(180deg, #251d16, #1a1410)", borderTop: "1px solid var(--border)", borderRadius: "16px 16px 0 0", padding: "24px 24px calc(24px + var(--safe-bottom))", animation: "sheetSlideUp 0.35s cubic-bezier(0.4,0,0.2,1)", WebkitOverflowScrolling: "touch" }}>
            ...
            <div style={{ fontSize: 14.5, lineHeight: 1.7, color: "var(--text-dim)", fontFamily: "var(--serif)", maxWidth: 520, margin: "0 auto" }}>
              <p style={{ marginBottom: 16, fontStyle: "italic", color: "var(--text)", fontSize: 15 }}>The Traveler's signal doesn't arrive as a single voice. It's reconstructed from interference patterns — fragments of four timelines, stitched together and grounded in what we can verify today. The signal reaches five years forward — long enough for real surprise.</p>
              ...
              <div style={{ marginTop: 24, paddingTop: 16, borderTop: "1px solid var(--border)", fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-faint)", lineHeight: 1.6, letterSpacing: "0.02em" }}>Tomorrow's Witness runs Claude Sonnet 4.6 (Anthropic) for three forecast lenses, GPT-4o (OpenAI) for an independent cross-model check, and Claude Opus 4.6 for final synthesis — grounded in superforecaster methodology, live web search, and prediction market data from{" "}<a href="https://polymarket.com" target="_blank" rel="noopener" style={{ color: "var(--amber-dim)", textDecoration: "none" }}>Polymarket</a>{" "}and{" "}<a href="https://kalshi.com" target="_blank" rel="noopener" style={{ color: "var(--amber-dim)", textDecoration: "none" }}>Kalshi</a>.{" "}The Traveler doesn't know the future — it extrapolates from patterns, probabilities, and current signals. The confidence ring is your honesty signal.</div>
            </div>
